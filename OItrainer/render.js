...
/* =========== Dottle立绘显示功能 =========== */
function showDottlePortrait(event) {
  // 获取李羿辰的学生对象
  let dottleStudent = null;
  let dottlePressure = 0;
  let isPermanentlyBlackened = false;
  
  try {
    if (typeof game !== 'undefined' && game.students) {
      dottleStudent = game.students.find(s => s && (s.name === '李羿辰' || s.name === '李羿辰（黑化）') && s.active !== false);
      if (dottleStudent) {
        dottlePressure = Number(dottleStudent.pressure || 0);
        isPermanentlyBlackened = dottleStudent._isBlackened || dottleStudent.name === '李羿辰（黑化）';
      }
    }
  } catch(e) {
    console.error('获取李羿辰状态失败:', e);
  }
  
  // 根据黑化状态和压力值决定使用哪个立绘
  // 如果已经永久黑化，总是显示黑化立绘；否则根据压力值判断
  const isBlackened = isPermanentlyBlackened || dottlePressure >= 75;
  const imageSrc = isBlackened ? 'dottle2.png' : 'dottle.png';
  const titleText = isPermanentlyBlackened 
    ? '李羿辰（黑化）- 魔女' 
    : (isBlackened ? '李羿辰 - 超能力者 [压力过高]' : '李羿辰 - 超能力者');
  const titleColor = isBlackened ? '#8B0000' : '#FFD700'; // 黑化时使用暗红色
  
  // 创建或获取立绘容器
  let portraitContainer = document.getElementById('dottle-portrait-container');
  if (!portraitContainer) {
    portraitContainer = document.createElement('div');
    portraitContainer.id = 'dottle-portrait-container';
    ...
    title.id = 'dottle-portrait-title';
    title.style.cssText = `
      text-align: center;
      font-size: 18px;
      font-weight: bold;
    ...
    portraitContainer.appendChild(title);
    document.body.appendChild(portraitContainer);
  }
  
  // 更新立绘和标题
  const img = document.getElementById('dottle-portrait-img');
  const title = document.getElementById('dottle-portrait-title');
  if (img) img.src = imageSrc;
  if (title) {
    title.textContent = titleText;
    title.style.color = titleColor;
    title.style.textShadow = isBlackened 
...
function hideDottlePortrait(event) {
  const portraitContainer = document.getElementById('dottle-portrait-container');
  if (portraitContainer) {
    portraitContainer.style.opacity = '0';
    setTimeout(() => {
      if (portraitContainer.parentNode) {
        portraitContainer.parentNode.removeChild(portraitContainer);
      }
    }, 300);
  }
}
...
function renderAll(){
  ...
  for(let s of game.students){
    ...
    const pressureLowThreshold = (typeof PRESSURE_LOW_THRESHOLD !== 'undefined' ? PRESSURE_LOW_THRESHOLD : 35);
    const pressureMidThreshold = (typeof PRESSURE_MID_THRESHOLD !== 'undefined' ? PRESSURE_MID_THRESHOLD : 65);
    let pressureLevel = s.pressure < pressureLowThreshold ? "低" : s.pressure < pressureMidThreshold ? "中" : "高";
    let pressureClass = s.pressure < pressureLowThreshold ? "pressure-low" : s.pressure < pressureMidThreshold ? "pressure-mid" : "pressure-high";
    ...
    out += `<div class="student-box" data-student-name="${s.name}">
  ...
  // 为李羿辰(dottle)添加立绘显示功能（包括黑化状态）
  document.querySelectorAll('#student-list .student-box').forEach(box => {
    const studentName = box.getAttribute('data-student-name');
    if(studentName === '李羿辰' || studentName === '李羿辰（黑化）'){
      box.style.cursor = 'pointer';
      box.addEventListener('mouseenter', showDottlePortrait);
      box.addEventListener('mouseleave', hideDottlePortrait);
    }
  });
...
function trainStudentsUI(){
  ...
  const taskCards = tasks.map((task, idx) => {
    ...
    // 计算题目难度对压力的影响提示
    const taskDifficultyFactor = task.difficulty / 100.0;
    const taskDiffPressFactor = (typeof TASK_DIFFICULTY_PRESSURE_FACTOR !== 'undefined' ? TASK_DIFFICULTY_PRESSURE_FACTOR : 10);
    const taskBasePressure = Math.floor(taskDifficultyFactor * taskDiffPressFactor);
    const pressureHint = task.difficulty < 40 ? '低压力' : task.difficulty < 80 ? '中等压力' : '高压力';
    const pressureColor = task.difficulty < 40 ? '#48bb78' : task.difficulty < 80 ? '#ecc94b' : '#f56565';
    ...
    <div class="prov-card option-card task-card" data-idx="${idx}" style="min-width:200px;padding:12px;border-radius:6px;cursor:pointer;border:2px solid #ddd;">
      ...
      <div class="small" style="margin:4px 0">难度: ${diffTag} <span style="color:${pressureColor};font-weight:600">(${pressureHint})</span></div>
      <div class="card-desc small muted" style="margin-bottom:4px">${boostStr}</div>
      <div class="small muted" style="font-size:11px;color:#999">难度基础压力: +${taskBasePressure}</div>
    </div>
  `;
  }).join('');
  ...
  showModal(`<h3>选择训练题目</h3>
    <div class="small muted" style="margin-bottom:10px;line-height:1.6">从下方5道题目中选择一道进行训练。<br>
    • <strong>知识点提升</strong>：每道题提升对应的知识点（如"数据结构+15"），实际效果受学生能力与题目难度的匹配度影响（10%-100%效率）<br>
    • <strong>压力增幅</strong>：题目难度越高，基础压力越大。简单题（难度&lt;40）低压力，中等题（40-80）中等压力，难题（&gt;80）高压力<br>
    • <strong>能力匹配</strong>：选择与学生能力接近的题目可获得最佳训练效率；太简单或太难的题目效率都会降低</div>
  ...
  $('mock-submit').onclick = ()=>{
    ...
    if(isPurchased){
      ...
      difficultyConfig = {
        ...
        maxScore: contestType.maxScore,
        problemScores: contestType.problemScores,
        subtasksPerProblem: contestType.subtasksPerProblem
      };
      numProblems = contestType.numProblems;
  ...
  game.recordExpense(costAdj, `娱乐活动：${opt.val}`);
    closeModal();
      for(let s of game.students){
        ...
        s.mental = Math.min((typeof MAX_MENTAL !== 'undefined' ? MAX_MENTAL : 100), s.mental);
        try{
          if(typeof s.triggerTalents === 'function'){
            ...
            for(const r of results){ if(!r || !r.result) continue; const out = r.result; if(typeof out === 'object'){
              ...
              if(out.action === 'vacation_half_minus5'){
                ...
                s.pressure = Math.min((typeof MAX_PRESSURE !== 'undefined' ? MAX_PRESSURE : 100), s.pressure + addBack);
...
function takeVacationUI(){
  ...
  let weeks = Math.ceil(days / 7);
    ...
    for(let s of game.students){
      if(!s || s.active === false) continue;
      s.mental = Math.min((typeof MAX_MENTAL !== 'undefined' ? MAX_MENTAL : 100), s.mental + days * uniform(3,8));
...
function initGameUI(){
  showModal(`<h3>欢迎 — OI 教练模拟器</h3>
    ...
    <label class="block">学生人数 (3-20)</label><input id="init-stu" type="number" min="3" max="20" value="5" />
  ...
  $('init-start').onclick = ()=>{
    ...
    let count = clampInt(parseInt($('init-stu').value),3,20);
    closeModal();
  ...
  let endingReason = normalizeEndingReason(rawEnding || (o.endingReason || o.oi_coach_ending_reason || '赛季结束'));
    ...
    if(o.students && o.students.length > 0) {
      ...
      for(let s of o.students) {
        ...
        const pressureLowThreshold = (typeof PRESSURE_LOW_THRESHOLD !== 'undefined' ? PRESSURE_LOW_THRESHOLD : 35);
        const pressureMidThreshold = (typeof PRESSURE_MID_THRESHOLD !== 'undefined' ? PRESSURE_MID_THRESHOLD : 65);
        const pressureLevel = (s && typeof s.pressure === 'number') ? (s.pressure < pressureLowThreshold ? '低' : s.pressure < pressureMidThreshold ? '中' : '高') : '—';
        const pressureClass = (s && typeof s.pressure === 'number') ? (s.pressure < pressureLowThreshold ? 'pressure-low' : s.pressure < pressureMidThreshold ? 'pressure-mid' : 'pressure-high') : '';
        ...
        studentsHtml += `<div class="student-box" style="margin-bottom:8px;padding:8px;background:white;border-radius:6px;border:1px solid #eee">
          <div class="student-header">
            ...
            <div class="student-status">
              <span class="label-pill ${pressureClass}">压力: ${s.pressure ? Math.floor(s.pressure) : 0} (${pressureLevel})</span>
            </div>
          </div>
...
function outingTrainingUI() {
    showModal(`<h3>外出集训</h3>
      ...
      <div class="talent-inspire-panel collapsible collapsed" style="margin-top:12px;margin-bottom:12px;padding:10px;border:1px solid #e2e8f0;border-radius:6px;background:#f7fafc">
        ...
        <div class="collapsible-content" style="margin-top:8px">
          <div class="small muted" style="margin-bottom:8px">每选择一个激发天赋消耗 ¥50,000，参加集训的学生有 30% 概率获得该天赋</div>
    ...
    if(outTalentGrid && window.TalentManager){
      ...
      allTalents.forEach(talentName => {
        ...
        // 过滤负面天赋和特有天赋（prob为0的天赋）
        if (talentDef.beneficial === false || talentDef.prob === 0.0) {
          return; // 跳过负面效果和特有效果
        }
    ...
    function updateOutingCostPreview(){
      ...
      const talentCost = selectedTalents * 50000;
