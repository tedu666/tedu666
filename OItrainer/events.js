  ...
  const EventManager = {
    ...
    registerDefaultEvents(ctx){
      ...
      this.register({
        ...
        run: c => {
          for(let s of c.game.students){
            ...
            s.pressure = Math.min((typeof MAX_PRESSURE !== 'undefined' ? MAX_PRESSURE : 100), oldP + 15);
      ...
      this.register({
        ...
        check: c => c.game.students.some(s => s.active && (s.pressure >= PRESSURE_QUIT_THRESHOLD || (s.quit_tendency_weeks && s.quit_tendency_weeks > 0))),
        run: c => {
          ...
          for(let i = c.game.students.length - 1; i >= 0; i--){
            ...
            // 压力>=PRESSURE_QUIT_THRESHOLD时获得或持续退队倾向
            if(s.pressure >= PRESSURE_QUIT_THRESHOLD){
              // 初始化退队倾向周数
              if(!s.quit_tendency_weeks) s.quit_tendency_weeks = 0;
              ...
              if(s.quit_tendency_weeks > 1){
                let prob = QUIT_PROB_BASE + QUIT_PROB_PER_EXTRA_PRESSURE * (s.pressure - PRESSURE_QUIT_THRESHOLD);
              ...
              // 压力降到PRESSURE_QUIT_THRESHOLD以下，清除退队倾向
              if(s.quit_tendency_weeks && s.quit_tendency_weeks > 0){
                // 调试日志
                if(typeof console !== 'undefined' && console.log){
    ...
    description: '知名教练莅临指导，学生能力提升，压力降低',
      ...
      this.register({
        ...
        run: c => {
          for(const s of c.game.students){
            if(!s || s.active === false) continue;
            const oldP = Number(s.pressure || 0);
            s.pressure = Math.min((typeof MAX_PRESSURE !== 'undefined' ? MAX_PRESSURE : 100), oldP + 20);
            try{ 
      ...
      this.register({
        ...
        run: c => {
          for (const s of c.game.students) {
            ...
            const oldP = Number(s.pressure || 0);
            s.pressure = Math.min((typeof MAX_PRESSURE !== 'undefined' ? MAX_PRESSURE : 100), oldP + c.utils.uniformInt(1, 4));
          ...
          const options = [
            { label: '接受邀请', effect: () => {
                ...
                for (const s of c.game.students) if (s.active) {
                  ...
                  const oldP = Number(s.pressure || 0);
                  s.pressure = Math.min((typeof MAX_PRESSURE !== 'undefined' ? MAX_PRESSURE : 100), oldP + 2);
        ...
        run: c => {
          const options = [
            { label: '高调宣传', effect: () => {
                ...
                for (const s of c.game.students) if (s.active) s.pressure = Math.min((typeof MAX_PRESSURE !== 'undefined' ? MAX_PRESSURE : 100), s.pressure + 10);
        ...
        run: c => {
          const options = [
            { label: '参加', effect: () => {
                ...
                c.game.budget += gain;
                for (const s of c.game.students) if (s.active) {
                  s.pressure = Math.min((typeof MAX_PRESSURE !== 'undefined' ? MAX_PRESSURE : 100), s.pressure + 10);
      ...
      this.register({
          ...
          try{
            ...
            for(const rec of relevant){
              ...
              for(const e of rec.entries){
                ...
                // NOIP 满分 或 省选 > 500
                if(rec.name === 'NOIP' && e.score >= 0.75 * noipFull) return true;
                if(rec.name === '省选' && e.score > 0.75 * 500) return true;
              }
            }
        ...
        run: c => {
          try{
            ...
            const maxRep = 100; // reputation上限保持100
            const maxMental = (typeof MAX_MENTAL !== 'undefined' ? MAX_MENTAL : 100);
            const repFactor = (rep / maxRep) * 0.5; // up to +0.5
            const mentalFactor = (mental / maxMental) * 0.25; // up to +0.25
            let successProb = base + repFactor + mentalFactor;
            const options = [
              { label: `消耗 ¥${cost}，全力挽留`, effect: () => {
                  ...
                  if(roll < successProb){
                    ...
                    stud.mental = Math.min((typeof MAX_MENTAL !== 'undefined' ? MAX_MENTAL : 100), Number(stud.mental || 0) + 5);
                    ...
                    for(const s of c.game.students){ if(!s || s.active === false) continue; s.pressure = Math.min((typeof MAX_PRESSURE !== 'undefined' ? MAX_PRESSURE : 100), Number(s.pressure || 0) + 10); }
      ...
      this.register({
        ...
        run: c => {
          ...
          const msg = `学校因财政紧张削减了预算，当前预算从 ¥${oldBudget.toLocaleString()} 降至 ¥${newBudget.toLocaleString()}（减少 ¥${reduction.toLocaleString()}）`;
          
          if (c.log) c.log(`[预算削减] ${msg}`);
          ...
          try {
            ...
            console.error('budget_cut pushEvent error', e);
          }
