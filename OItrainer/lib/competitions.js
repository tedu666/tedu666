/* competitions.js - 比赛模拟引擎
   重构后的比赛系统：采用逐步模拟（每5分钟一次tick）的方式计算学生解题过程
   
   核心概念：
   - 每道题有3-5档部分分（subtask），难度递增
   - 学生在每个时间片(5分钟)进行：选题 -> 思考/尝试 -> 可能跳题
   - 分数实时累积，最终得到比赛总分
   
   时间流逝：网页1秒 = 比赛5分钟
   
   比赛时长：
   - CSP-S1: 120分钟 (24个tick)
   - CSP-S2/NOIP: 240分钟 (48个tick)
   - 省选/NOI: 480分钟 (96个tick)
*/
...
(function(global){
  ...
  const CONTEST_DURATION = {
    ...
    'Atcoder ARC': 120,
    'Atcoder AGC': 150,
    'Codeforces Div.3': 120,
    'Codeforces Div.2': 120,
    'Codeforces Div.1': 140,
    '力扣周赛': 60, 
    '梦熊周赛': 240, 
    '洛谷月赛': 240, 
    "测试-耐久赛": 2000,
  ...
  function generateSubtasks(totalScore, problemDifficulty, thinkingBase=null, codingBase=null, options={}){
    ...
    const numSubtasks = forceSingle ? 1 : 
                        ...
                        (3 + Math.floor(getRandom() * 30)); // 1 或 3-5 档 或 指定数量
    const subtasks = [];
  ...
  class ContestSimulator {
      ...
      for(let st of this.students){
        ...
        if(typeof s.triggerTalents === 'function'){
          try{ 
            ...
            const results = s.triggerTalents('contest_start', { contestName: this.config.name, state: st, simulator: this }) || [];
    ...
    simulateStudentTick(state){
      ...
      if(needsNewTarget){
        ...
        // CSP-S1: 选择新题时重置失败计数
        if(this.config.name === 'CSP-S1'){
          const newProb = state.getProblem(selected);
          if(newProb) newProb.csp_fail_count = 0;
        }
      ...
      if(success){
        ...
        // CSP-S1: 失败后增加失败计数（用于动态调整阈值）
        if(this.config.name === 'CSP-S1'){
          if(!prob.csp_fail_count) prob.csp_fail_count = 0;
          prob.csp_fail_count++;
          this.addLog(`${s.name} T${state.currentTarget + 1} 失败${prob.csp_fail_count}次，下次阈值：${Math.min(100, 40 + prob.csp_fail_count * 10)}%`, 'info', s.name);
        }
        ...
        if(shouldSkip){
          ...
          // CSP-S1: 跳题时重置失败计数
          if(this.config.name === 'CSP-S1' && prob.csp_fail_count){
            prob.csp_fail_count = 0;
          }
    ...
    selectProblem(state, student){
      ...
      // CSP-S1 特殊策略：完全随机选题
      if(this.config.name === 'CSP-S1'){
        const randomIndex = Math.floor(getRandom() * unsolved.length);
        return unsolved[randomIndex].id;
      }
    ...
    selectBestSubtask(student, prob){
      ...
      // CSP-S1 特殊策略：保守做题，动态调整阈值
      if(this.config.name === 'CSP-S1'){
        // 初始化失败次数记录
        if(!prob.csp_fail_count) prob.csp_fail_count = 0;
        
        // 过滤出还没拿到分数的档位（分数比当前最高分更高的）
        const higherSubtasks = availableSubtasks.filter(item => item.subtask.score > prob.maxScore);
        
        if(higherSubtasks.length === 0){
          // 已经AC或没有更高档位，返回最后一档
          return availableSubtasks[availableSubtasks.length - 1].idx;
        }
        
        // 按分值从高到低排序（优先考虑高分档位）
        const sortedByScore = higherSubtasks.slice().sort((a, b) => {
          return b.subtask.score - a.subtask.score; // 从高分到低分
        });
        
        // 计算有效能力
        const effectiveAbility = (thinking + coding) / 2 + knowledge * 0.4;
        
        // 根据失败次数动态调整阈值
        // 失败0次: 40%, 失败1次: 50%, 失败2次: 60%, ..., 失败6次及以上: 100%
        const requiredProb = Math.min(1.0, 0.4 + prob.csp_fail_count * 0.1);
        
        // 从分值最高的档位开始，找第一个通过率>阈值的档位
        for(let item of sortedByScore){
          const st = item.subtask;
          const avgDiff = (Number(st.thinkingDifficulty || st.difficulty || 0) + 
                          Number(st.codingDifficulty || st.difficulty || 0)) / 2;
          
          // 估算通过概率（简化版）
          const gap = effectiveAbility - avgDiff;
          const estimatedProb = 1.0 / (1.0 + Math.exp(-gap / 15.0));
          
          if(estimatedProb >= requiredProb){
            // 通过率满足阈值，选择这个档位
            return item.idx;
          }
        }
        
        // 如果所有档位通过率都不满足，选择通过率最高的档位（最简单的）
        let bestItem = sortedByScore[0];
        let bestProb = 0;
    ...
    attemptSubtask(student, problem, subtask){
      ...
      const thinkingStability = 0.75 + 0.25 * (mental / maxMental);
      ...
      const maxMental2 = (typeof MAX_MENTAL !== 'undefined' ? MAX_MENTAL : 100);
      const codingStability = 0.8 + 0.2 * (mental / maxMental2);
  ...
  function buildContestConfig(contestDef){
    ...
    // 验证 problemScores 配置
    if(contestDef.problemScores && Array.isArray(contestDef.problemScores)){
      const sumScores = contestDef.problemScores.reduce((sum, score) => sum + Number(score || 0), 0);
      if(sumScores !== contestDef.maxScore){
        console.warn(`[比赛配置警告] ${contestDef.name}: problemScores总和(${sumScores})与maxScore(${contestDef.maxScore})不符，将使用problemScores的总和`);
        contestDef.maxScore = sumScores; // 自动修正
      }
    }

    for(let i = 0; i < contestDef.numProblems; i++){
      // 支持为每道题指定不同的满分
      // 如果提供了 problemScores 数组，使用指定的分数；否则平均分配
      let problemScore;
      if(contestDef.problemScores && Array.isArray(contestDef.problemScores) && contestDef.problemScores[i] !== undefined){
        problemScore = Number(contestDef.problemScores[i]);
      } else {
        problemScore = Math.floor(contestDef.maxScore / contestDef.numProblems);
      }
      ...
      // 应用游戏难度增幅（根据游戏难度调整比赛难度）
      const difficultyMultiplier = (typeof DIFFICULTY_MULTIPLIER !== 'undefined' ? DIFFICULTY_MULTIPLIER : 1.0);
      rawProblemDifficulty = rawProblemDifficulty * difficultyMultiplier;
      ...
      // 检查是否指定了subtask数量
      // 支持两种方式：
      // 1. subtasksPerProblem: Number - 所有题目统一的subtask数量
      // 2. subtasksPerProblem: Array - 为每道题指定不同的subtask数量
      let numSubtasks = null;
      if(contestDef.subtasksPerProblem !== undefined && contestDef.subtasksPerProblem !== null){
        if(Array.isArray(contestDef.subtasksPerProblem)){
          // 数组形式：为每道题指定
          numSubtasks = contestDef.subtasksPerProblem[i] || null;
        } else {
          // 数字形式：统一应用
          numSubtasks = contestDef.subtasksPerProblem;
        }
      }
  ...
  function getOnlineContestDifficultyFactors(contestType, numProblems){
    ...
    // Atcoder ARC: 4题，从120到350，难度递增
    if(contestType === "Atcoder-AGC"){
      return [0.62, 0.9, 1.32, 1.6];
    }
    ...
    if(contestType === "梦熊周赛"){
      // 使用类似 Div2 的分布，但4题：0.5, 0.8, 1.2, 1.5
      return [0.6, 1, 1.4, 1.6];
    }

    // Ucup: 13题，省选级难度(320)
    if(contestType === "Ucup"){
      // 使用类似 Div1 的分布，但4题：0.5, 0.8, 1.2, 1.5
      return [0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 1.6];
    }
