...
(function(global){
  ...
  function enterNationalTeam(goldStudents) {
    ...
    const nationalTeamComps = [
      ...
      {week: currentWeek + 4, name:"CTS", difficulty:520, maxScore:600, numProblems:6, nationalTeam:true}
  ...
  function calculateNationalTeamQualification() {
    ...
    // 计算晋级线（总分的50%）
    const maxPossible = 600 + 600 + 600; // CTT-day1-2 + CTT-day3-4 + CTS
    ...
    for(let i = 0; i < Math.min(4, totalScores.length); i++){
      if(totalScores[i].total >= passLine){
        // 检查王相文的"临门一脚"天赋
        const studentName = totalScores[i].name;
        const student = game.students ? game.students.find(s => s && s.name === studentName) : null;
        
        let shouldBlock = false;
        if(student && student.talents && student.talents.has('临门一脚')){
          // 检查距离晋级线的距离
          const gap = totalScores[i].total - passLine;
          if(gap >= 0 && gap <= 50){
            // 距离晋级线50分以内，触发天赋效果
            shouldBlock = true;
            console.log(`[临门一脚] ${studentName} 距离晋级线仅${gap.toFixed(0)}分，因天赋效果无法晋级`);
            
            // 推送事件通知
            if(typeof global.pushEvent === 'function'){
              global.pushEvent({
                name: '临门一脚',
                description: `${studentName} 距离晋级线仅${gap.toFixed(0)}分，但因"临门一脚"天赋无法晋级IOI`,
                week: game.week
              });
            }
          }
