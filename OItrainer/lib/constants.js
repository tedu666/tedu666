...
const COMPETITION_SCHEDULE = [
  ...
  {week:54, name:"CTT-day1-2", difficulty:570, maxScore:600, numProblems:6, nationalTeam:true},
  {week:55, name:"CTT-day3-4", difficulty:580, maxScore:600, numProblems:6, nationalTeam:true},
  {week:56, name:"CTS", difficulty:630, maxScore:600, numProblems:6, nationalTeam:true},
  {week:57, name:"IOI", difficulty:700, maxScore:600, numProblems:6, nationalTeam:true}
];
...
/* 学生能力范围 */
const STRONG_PROVINCE_MIN_ABILITY = 30.0;
const STRONG_PROVINCE_MAX_ABILITY = 50.0;
const NORMAL_PROVINCE_MIN_ABILITY = 20.0;
const NORMAL_PROVINCE_MAX_ABILITY = 40.0;
const WEAK_PROVINCE_MIN_ABILITY = 20.0;
const WEAK_PROVINCE_MAX_ABILITY = 45.0;

//初始能力
const KNOWLEDGE_ABLILTY_START = 70;

/* 难度修正 */
const EASY_MODE_BUDGET_MULTIPLIER = 1.5;  // 简单模式：预算增加50%（原1.15）
const HARD_MODE_BUDGET_MULTIPLIER = 0.7;  // 困难模式：预算减少50%（原0.7）
const EASY_MODE_ABILITY_BONUS = 10.0;     // 简单模式：能力加成翻倍（原10.0）
...
// 题目难度对压力的影响系数
const TASK_DIFFICULTY_PRESSURE_FACTOR = 10;  // 题目难度对压力的基础系数（difficulty/100 * 此系数）
const TASK_MISMATCH_PRESSURE_FACTOR = 0.2;   // 题目与学生能力不匹配时的额外压力系数
// 训练基础压力值（根据强度）
const TRAINING_BASE_PRESSURE_LIGHT = 15;     // 轻度训练的基础压力
const TRAINING_BASE_PRESSURE_MEDIUM = 25;    // 中等训练的基础压力
const TRAINING_BASE_PRESSURE_HEAVY = 40;     // 重度训练的基础压力
...
const CONTEST_GAIN_RATIOS = {
  // 正式比赛
  'CSP-S1': { knowledge: 0.4, thinking: 0.4, coding: 0.3 },      // 入门级
...
/* 外出集训 */
const OUTFIT_BASE_COST_BASIC = 34000;
const OUTFIT_BASE_COST_INTERMEDIATE = 50000;
const OUTFIT_BASE_COST_ADVANCED = 140000;
...
const ONLINE_CONTEST_TYPES = [
  ...
  {name: "洛谷月赛", numProblems: 4, difficulty: 250, displayName: "洛谷月赛"},
...
/* 娱乐 */
const ENTERTAINMENT_COST_MEAL = 5000;
...
/* 压力等级阈值 */
const PRESSURE_LOW_THRESHOLD = 90;    // 压力低于此值为"低压力"
const PRESSURE_MID_THRESHOLD = 170;    // 压力低于此值为"中压力"，否则为"高压力"
const PRESSURE_HIGH_WARNING = 210;     // 压力达到此值时显示高压力警告（训练预览）
const PRESSURE_QUIT_THRESHOLD = 280;   // 压力达到此值时触发退队倾向
const QUIT_PROB_BASE = 0.22;
const QUIT_PROB_PER_EXTRA_PRESSURE = 0.02;
const TALENT_LOST_VALUE = 220;         // 触发丧失天赋的压力阈值
/* 劝退消耗声誉 */
