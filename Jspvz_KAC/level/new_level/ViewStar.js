...
oS.Init({
	...
	LoadAccess: function(LevelDefFuc) {
		for(i in oS.TaskPlants){
			...
			NewImg("PlantAlpha_"+y+"_"+x+"_"+AlphaPlants.i.prototype.PKind, j.PicArr[j.StaticGif], "visibility:visible;filter:alpha(opacity=40);opacity:0.4;z-index:3;left:" + (GetX(x) - j.width/2) + "px;top:" + (GetY(y) - j.height - 10) + "px;" , EDAll);
		};
		LevelDefFuc();
	...
	StartGame: function() {//开始游戏
		...
		PrepareGrowPlants(function() {
			...
			AutoProduceSun(25);
			NewEle("DivTeach", "div", 0, {innerHTML: "用对应植物塞满被围出的空间吧！"}, EDAll);
