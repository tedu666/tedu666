var CZombies = function(b, a) {
	return (a = function() {}).prototype = {
		...
		Birth: function() {
			var c = this;
			this.Async_Picture(), Trigger_Zombies_Birth(this); //异步
			$Z[c.id] = c;
			oZ.add(c);
			c.BirthCallBack(c)
		},
		...
		NormalDie: function() {
			...
			c.HP = 0;
			delete $Z[c.id];
			c.PZ && (oP.MonPrgs(), Trigger_Zombies_Die(c));
		},
		...
		DisappearDie: function() {
			...
			this.HP = 0;
			delete $Z[this.id];
			this.PZ && (oP.MonPrgs(), Trigger_Zombies_Die(this));
		},
		CrushDie: function() {
			var c = this;
		...
		bedevil: function(c) {
			...
			(oP.MonPrgs(), Trigger_Zombies_Die(c));
		},
...
oBackupDancer = InheritO(OrnNoneZombies, {
	...
	bedevil: function(a) {
		...
		(oP.MonPrgs(), Trigger_Zombies_Die(a));
	},
...
oDancingZombie = InheritO(OrnNoneZombies, {
	...
	NormalDie: function() {
		...
		a.HP = 0;
		delete $Z[a.id];
		a.PZ && (oP.MonPrgs(), Trigger_Zombies_Die(a));
	},
	ExplosionDie: function() {
		var a = this;
		a.ResetBackupDancer(a);
	...
	bedevil: function(b) {
		...
		(oP.MonPrgs(), Trigger_Zombies_Die(b));
	},
...
oZomboni = function() {
	...
	return InheritO(OrnNoneZombies, {
		...
		NormalDie: function() {
			...
			delete $Z[b.id];
			b.JudgeIce();
			b.PZ && (oP.MonPrgs(), Trigger_Zombies_Die(b));
		},
		DisappearDie: function() {
			var b = this;
			ClearChild(b.Ele);
...
oBalloonZombie = InheritO(OrnI_Norn_Zombies, {
		...
		function(b) {
			...
			if (c) {
				...
				c.ExplosionDie = function() {
					...
					d.HP = 0;
					delete $Z[d.id];
					d.PZ && (oP.MonPrgs(), Trigger_Zombies_Die(d)); this.Die_Out();
				};
				c.DisappearDie = function() {
					ClearChild(this.Ele);
					this.HP = 0;
					delete $Z[this.id];
					this.PZ && (oP.MonPrgs(), Trigger_Zombies_Die(this)); this.Die_Out();
				};
				c.CrushDie = function() {
					var d = this;
					...
					d.HP = 0;
					delete $Z[d.id];
					d.PZ && (oP.MonPrgs(), Trigger_Zombies_Die(d)); d.Die_Out();
				}
			}
		},
		[a.id])
	...
	NormalDie: function() {
		...
		a.HP = 0;
		delete $Z[a.id];
		a.PZ && (oP.MonPrgs(), Trigger_Zombies_Die(a)); a.Die_Out();
	},
	ExplosionDie: function() {
		var a = this;
