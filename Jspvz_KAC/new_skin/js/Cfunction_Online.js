...
var __NowVer__ = 20230801; // 八月一预更新留版本号
...
let __Ctk_User_PassWord_ = async function(){
	...
	await _AJAX_("https://tedu666.rth1.vip/php/login.php", "POST", "Password=" + window["OL_PassWord"], function(ret){
...
let __GetUrl__ = (async function(){
	return "https://kac-jspvz.rth1.vip/online/Level_List.json";
...
(async function(){
	...
	await _AJAX_("https://tedu666.rth1.vip/php/Get_New_Ver.php", "POST", "", function(f){
		if (Number(f) > Number(__NowVer__)) w = confirm("检测到可能有新版本，是否跳转至下载链接？");
		if (w) window["open"]("https://www.luogu.com.cn/paste/1rhbg5ok");
	});

	await _AJAX_("https://tedu666.rth1.vip/php/Get_Notice.php", "POST", "", function(f){ // 可能会有的公告、补丁
		(f != "") && eval(f);
	});
})();
