...
(!window["oZombossW"]) && (oZombossW = InheritO(oZombie, {
	...
	Produce: '韧性：<font color="#FF0000">不死之身</font></p>由白鹤亮翅设计，我是帅打造的大脑号仿尸机器人W型',
	Birth: function() {
		var c = this;
		$Z[c.id] = c;
		oZ.add(c);
	...
	Skill_List: [
{
	...
	func: function(dboss = {}, num, This){
		for(let i in $Z){
			let d = $Z[i], l = GetX(GetC(d.ZX)) - GetX(GetC(d.ZX) - 1);
...
},
{
	Name: "喜怒无常",
	EName: "GOOD_LUCK",
	Produce: "50%概率僵尸加3.2速度加200攻击,boss减1500血; 50%概率boss加2500血",
	weight: 20,
	func: function(dboss = {}, num, This){
		let ram = Math.floor(1 + Math.random() * 100);
		let bossadd = (ram <= 50 ? -1500 : 2500);
		for(let i in $Z){
			let d = $Z[i];
			if(d.EName == dboss.EName) continue;
			if(ram <= 50) d.OSpeed += 3.2, d.Speed += 3.2, d.Attack += 200;
		};
		dboss.HP += bossadd;
	}
},
	...
	func: function(dboss = {}, num, This){
		...
		oSym.addTask(1, function(canvas, ctx, alpha, kind){
			...
			if(kind == 0 && alpha <= 1){
				...
				if(kind == 0){
					let M = {}, Arr; Object.values($P)["map"]((O) => O.R >= 1 && O.C >= 1 && (M[O.EName] |= 0, ++M[O.EName]));
					Arr = Object.keys(M), Arr.sort((A, B) => M[B] - M[A]);
					if (Arr.length) {
						let tmp = window[Arr[0]];
						for(let i = 1; i <= oS.R; i++){
							for(let j = 1; j <= oS.C; j++){
								let d = oGd.$[i + '_' + j + '_1']; if(!d) continue;
...
},
{
	Name: "自相矛盾",
	EName: "MAKE_FOG",
	Produce: "所有植物周围3x3的植物每秒受到雾气7伤害，持续10秒",
	weight: 15,
	func: function(dboss = {}, num, This){
		let Show_canvas = document.getElementById('CanvasC'), Show_ctx = Show_canvas.getContext('2d');
		let fogNum = 9, a = 2 * fogNum + 3, twx = GetX(-1) - 30, globalAlpha = 0.4, TimeTag = true;
		...
		//检测是否到下一技能，并且释放霾的伤害
		oSym.addTask(1000, () => (TimeTag = false), []);
