// alert("系统升级原因，本关暂停开放！"); oS.Init({}, {}, {}), SelectModal(0);
...
oS.Init({
	...
	GotoNextWave : function(survival_times){
		...
		NewEle("DivTeach", "div", 0, {innerHTML: "更多的僵尸要来了！"}, EDMove);//文字
		setTimeout(function(){
			ResetGame($("dMenu0"), 1);
	...
	GotoNextStartGame : function(h){
		...
		oS.lastwavenow = oSym.Now , oS.lastwaveTQ = oS.lastwaveTQ = oSym.TQ.AVL.values().flat().concat(oSym.AddTaskQ) , oSym.Clear();//保存TQ、清除TQ
		...
		oSym.addTask(h == undefined ? 200 : h,
			function(j) {
				...
				ClearChild(j); EDPZ.style.left = EDMove.style.left = "115px";
				i.ScrollScreen();
			},[
