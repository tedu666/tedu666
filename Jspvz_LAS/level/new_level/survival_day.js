...
false && oS.Init({
	...
	reload_last_view : function(){
		...
		for(i in oS.lastwaveTQ.heap){
			oSym.TQ.push(oS.lastwaveTQ.heap[i]);
		}
	},
	PauseGames : function() {
	...
	FindClearoSymTQ : function(str){
		let zombieaudio = [];
		for(item in oSym.TQ.heap){
			if(oSym.TQ.heap[item].f.toString().indexOf(str) != -1){
				zombieaudio.push(item);
			}
		}
		while(zombieaudio.length){
			oSym.TQ.heap.slice(zombieaudio[zombieaudio.length-1], 1);
			zombieaudio.length--;
		}	
	},
	FindClearoSymAr : function(str){
		let zombieaudio = [];
		for(item in oSym.TQ.heap){
			if(oSym.TQ.heap[item].ar.toString().indexOf(str) != -1){
				zombieaudio.push(item);
			}
		}
		while(zombieaudio.length){
	...
	GotoNextStartGame : function(h){
		...
		oS.lastwavenow = oSym.Now , oS.lastwaveTQ = oSym.TQ , oSym.Clear();//保存TQ、清除TQ
