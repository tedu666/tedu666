oS.Init({
	...
	LoadAccess: function(start_game){
		...
		NewEle("Div_TimeTask", "div", "display:none;z-index:5;top:10px; left:315px; position:absolute;width:355px;height:35px;background:#000000BB;border-radius:12.5px;pointer-events:none;opacity:1;", 0, EDMove);
	...
	FlagToEnd: function() {
		...
		NewImg("imgSF", "new_skin/Images/Card/MelonPult.webp", "height:120px;left:627px;top:325px;clip:rect(auto,auto,60px,auto)", EDMove, {onclick: function() {GetNewCard(this, oMelonPult, 0);}});
...
for(let i = 0; i < ArCard.length; i++){
	...
	NewImg(id, j.PicArr[j.StaticGif], "left:0px;top:0px;z-index:254", EDMove).cloneNode(false);
