var CZombies = function(b, a) {
	return (a = function() {}).prototype = {
		...
		CanPass: function(d, c) {
			return (c && c != 2) || (c == Infinity);
		},
		CanGrow: function(d, c, e) {
			return this.CanPass(c, oGd.Get_LF(c, e)) && (oS.ArP ? (e > oS.ArP.ArC[1]) : true);
		},
		...
		Birth: function() {
			var c = this;
			this.Async_Picture(), oGT.OnTrigger("ZombieBirth", this); //异步
			$Z[c.id] = c;
			oZ.add(c);
			c.BirthCallBack(c)
		},
		...
		getSlow: function(h, f, g) {
			...
			function(j, i) {
				var k = $Z[j];
				k && k.FreeSlowTime == i && (k.FreeSlowTime = 0, k.Attack = k.OAttack, k.Speed && (k.Speed = k.OSpeed))
			},
			[f, d])
		},
		getFreeze: function(d, c) {
			...
			function(g, f, e) {
				ClearChild(e);
				var h = $Z[g];
				h && h.FreeFreezeTime == f && (h.FreeFreezeTime = 0, h.Attack = (h.OAttack / 2), !h.FreeSetbodyTime && (h.Speed = 0.5 * h.OSpeed, h.isAttacking && h.JudgeAttack()), oSym.addTask(1500,
				function(j, i) {
					var k = $Z[j];
					k && k.FreeSlowTime == i && (k.FreeSlowTime = 0, k.Attack = k.OAttack, !k.FreeSetbodyTime && (k.Speed = k.OSpeed))
				},
		...
		NormalDie: function() {
			...
			c.HP = 0;
			delete $Z[c.id];
			c.PZ && (oP.MonPrgs(), oGT.OnTrigger("ZombieDie", c));
		},
		...
		DisappearDie: function() {
			...
			this.HP = 0;
			delete $Z[this.id];
			this.PZ && (oP.MonPrgs(), oGT.OnTrigger("ZombieDie", this));
		},
		CrushDie: function() {
			var c = this;
		...
		bedevil: function(c) {
			...
			(oP.MonPrgs(), oGT.OnTrigger("ZombieDie", c));
		},
...
OrnNoneZombies = function() {
	...
	return InheritO(CZombies, {
		...
		getSnowPea: function(f, c, g, ftime) {
			var wt = ftime || 1000, e = f.FreeSlowTime, b = oSym.Now + wt;
			e == 0 ? (f.PlaySlowballAudio(), f.Speed = 0.5 * f.OSpeed, f.Attack = (f.OAttack / 2)) : f.PlayNormalballAudio();
			e < b && (f.FreeSlowTime = b, oSym.addTask(wt, function(h, d) {
				var i = $Z[h];
				i && i.FreeSlowTime == d && (i.FreeSlowTime = 0, i.Attack = i.OAttack, i.Speed && (i.Speed = i.OSpeed))
			}, [f.id, b]));
			f.getHit0(f, c, g)
		}
	})
...
oBackupDancer = InheritO(OrnNoneZombies, {
	...
	Speed: 3.5,
	OSpeed: 3.5,
	HeadTargetPosition: [{x: 82, y: 13}, {x: 82, y: 15}],
	PicArr: (function() {
	...
	bedevil: function(a) {
		...
		(oP.MonPrgs(), oGT.OnTrigger("ZombieDie", a));
	},
	getSlow: function(f, d, e) {
		...
		function(h, g) {
			var i = $Z[h];
			i && i.FreeSlowTime == g && (i.FreeSlowTime = 0, i.Attack = i.OAttack)
		},
		[d, b])
	},
	getFreeze: function(b, a) {
		...
		function(e, d, c) {
			ClearChild(c);
			var f = $Z[e];
			f && f.FreeFreezeTime == d && (f.FreeFreezeTime = 0, f.Attack = (f.OAttack / 2), !f.FreeSetbodyTime && f.isAttacking && f.JudgeAttack(), oSym.addTask(1500,
			function(h, g) {
				var i = $Z[h];
				i && i.FreeSlowTime == g && (i.FreeSlowTime = 0, i.Attack = i.OAttack)
			},
...
oDancingZombie = InheritO(OrnNoneZombies, {
	...
	PicArr: (function() {
		var d = "images/Zombies/DancingZombie/", c = "",
	...
	NormalDie: function() {
		...
		a.HP = 0;
		delete $Z[a.id];
		a.PZ && (oP.MonPrgs(), oGT.OnTrigger("ZombieDie", a));
	},
	ExplosionDie: function() {
		var a = this;
		a.ResetBackupDancer(a);
	...
	bedevil: function(b) {
		...
		(oP.MonPrgs(), oGT.OnTrigger("ZombieDie", b));
	},
...
oFlagZombie = InheritO(oZombie, {
	...
	HeadTargetPosition: [{x: 80, y: 10}, {x: 80, y: 10}],
...
oConeheadZombie = InheritO(OrnIZombies, {
	...
	SunNum: 75,
	StandGif: 11,
	HeadTargetPosition: [{x: 72, y: 20}, {x: 72, y: 16}],
	PicArr: (function() {
...
oFootballZombie = InheritO(oConeheadZombie, {
	...
	HeadTargetPosition: [{x: 50, y: 10}, {x: 60, y: 15}],
	PlayNormalballAudio: function() {
		PlayAudio("plastichit")
	},
	PicArr: (function() {
...
oPoleVaultingZombie = InheritO(OrnNoneZombies, {
	...
	HeadTargetPosition: [{x: 192, y: 79}, {x: 192, y: 81}],
	StandGif: 13,
	GetDX: function() {
		return - 238
	},
...
oNewspaperZombie = InheritO(OrnIIZombies, {
	...
	LostPaperSpeed: 4.8,
	HeadTargetPosition: [{x: 62, y: 25}, {x: 60, y: 25}],
	PicArr: (function() {
...
oScreenDoorZombie = InheritO(oNewspaperZombie, {
	...
	getHit0: function(c, a, b) {
		...
		[c.id])) : (c.HP -= a) < c.BreakPoint && (c.GoingDie(c.PicArr[[c.LostHeadGif, c.LostHeadAttackGif][c.isAttacking]]), c.getHit = c.getHit0 = c.getHit1 = c.getHit2 = c.getHit3 = c.GoingDieGetHit)
	},
	CheckOrnHP: function(g, h, d, c, f, b, a) {
	...
	getFireball: function(c, a, b) {
		b != c.WalkDirection ? (c.FreeSlowTime = 0, c.Attack = c.OAttack, c.Speed != c.OSpeed ? (c.PlayNormalballAudio(), c.Speed = c.OSpeed) : c.PlayFireballAudio()) : c.PlayNormalballAudio()
	},
...
oZomboni = function() {
	...
	return InheritO(OrnNoneZombies, {
		...
		ChkActs: function(e, j, q, k) {
			...
			;;if (l == null) {
				(f = NewEle("dIceCar" + j, "div", "position:absolute;z-index:1;left:145px;top:" + (GetY(e.R) - 65) + "px;width:800px;height:72px", 0, EDPZ),
			...
			;;;e.JudgeAttack();
			(r = e.AttackedRX -= (b = e.Speed)) < -50 ? (q.splice(k, 1),
			e.DisappearDie(),
			m = 0) : (r < 100 && !e.PointZombie && (e.PointZombie = 1,
			!oS.CardKind && (StopMusic(),
		...
		NormalDie: function() {
			...
			delete $Z[b.id];
			b.JudgeIce();
			b.PZ && (oP.MonPrgs(), oGT.OnTrigger("ZombieDie", b));
		},
		DisappearDie: function() {
			var b = this;
			ClearChild(b.Ele);
...
oDolphinRiderZombie = InheritO(oAquaticZombie, {
	...
	width: 282,
	height: 210,
	HeadTargetPosition: [{x: 180, y: 105}, {x: 180, y: 105}],
	getShadow: function(a) {
	...
	NormalAttack: function(d, b, g) {
		...
		function(m, j, i, l, q) {
			...
			n = function() {
				q.src = h.PicArr[10];
				h.HeadTargetPosition = [{x: 43, y: 140}, {x: 46, y: 142}];
				h.isAttacking = 0;
				h.Altitude = 1;
...
oImp = InheritO(OrnNoneZombies, {
	...
	OSpeed: 5.4,
	Speed: 5.4,
	HeadTargetPosition: [{x: 20, y: 29}, {x: 23, y: 30}],
	GetDX: function() {
		return - 50
	},
	GetDY: function() {
...
oJackinTheBoxZombie = InheritO(OrnNoneZombies, {
	...
	OSpeed: 3.6,
	Speed: 3.6,
	HeadTargetPosition: [{x: 92, y: 10}, {x: 95, y: 10}],
			...
			function(f) {
				...
				e && (d = NewImg("", "images/interface/blank.png", "width:306px;height:300px;left:" + (e.X - 16) + "px;top:" + (e.pixelTop - 90) + "px;z-index:20"), PlayAudio("explosion"), d.src = e.PicArr[8] + Math.random(), EDPZ.appendChild(d), oSym.addTask(70, ClearChild, [d]), e.PZ ? ((function(k, g) {
					...
					do {
						...
						do {
							j = q + "_" + g + "_";
							for (l = 0; l < MAX_PLT_INDEX; l++) { (m = r[j + l]) && m.BoomDie()
							}
						} while ( g ++< h )
					} while ( q ++< o )
...
OrnI_Norn_Zombies = function() { // 只会打到本体
	var a = OrnNoneZombies.prototype.getHit, b = OrnIZombies.prototype.getHit;
	return InheritO(OrnIZombies, {getHit: a, getHit0: a, getHit1: a, getHit2: a, getHit3: a, getHit_Orn: b});
} (),
oBalloonZombie = InheritO(OrnI_Norn_Zombies, {
	...
	Die_Out: function() { // 死亡减去气球数
		...
		this.Is_Die_Out = true, oGd.$Balloon[this.R] |= 0, --oGd.$Balloon[this.R];
	},
	getShadow: function(a) {
		...
		function(b) {
			...
			if (c) {
				...
				c.ExplosionDie = function() {
					...
					d.HP = 0;
					delete $Z[d.id];
					d.PZ && (oP.MonPrgs(), oGT.OnTrigger("ZombieDie", d)); this.Die_Out();
				};
				c.DisappearDie = function() {
					ClearChild(this.Ele);
					this.HP = 0;
					delete $Z[this.id];
					this.PZ && (oP.MonPrgs(), oGT.OnTrigger("ZombieDie", this)); this.Die_Out();
				};
				c.CrushDie = function() {
					var d = this;
					...
					d.HP = 0;
					delete $Z[d.id];
					d.PZ && (oP.MonPrgs(), oGT.OnTrigger("ZombieDie", d)); d.Die_Out();
				}
			}
		},
		[a.id])
	...
	getFreeze: function(b, a) {
		b.Attack = (b.OAttack / 2);
		b.Speed = 0.5 * b.OSpeed;
		oSym.addTask(1500,
		function(d, c) {
			var e = $Z[d];
			e && e.FreeSlowTime == c && (e.FreeSlowTime = 0, e.Attack = e.OAttack, e.Speed = e.OSpeed)
		},
	...
	NormalDie: function() {
		...
		a.HP = 0;
		delete $Z[a.id];
		a.PZ && (oP.MonPrgs(), oGT.OnTrigger("ZombieDie", a)); a.Die_Out();
	},
	ExplosionDie: function() {
		var a = this;
