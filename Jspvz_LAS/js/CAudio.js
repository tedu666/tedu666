...
var HowlAudio = class { // 音频接口，有待完善
	Sound = void 0; Onload = false; PLines = null; Lines = null; MaxLine = 16; LastPlay = null; MinPlay = 5; AudioName = null; // 目前是 5 毫秒内
	constructor (Json) { let self = this, MLine = Json["line"] || 16; self["AudioName"] = Json["AudioName"], self["Sound"] = new Howl(Json), self["MaxLine"] = MLine, self["Lines"] = [], self["PLines"] = []; };
	isUrl(Str) {
	...
	play (Cofig = {}) { // 播放
		let self = this, ID = void 0; if (!self["Sound"]) return false; // 播放中或者不存在
		let { CallBack = (() => {}), Sprite = (void 0) } = Cofig, D = oSym["SysTime"](), b = self["isUrl"](self["Sound"]["_src"]);
	...
	stop (a, ID) { // 停止
		...
		if (self["Onload"] && self["Sound"]["state"]() == "loaded") {
			if (a) self["Sound"]["stop"](), self["freshline"](), self["PLines"]["length"] = 0; // 否则强制卸载
			self["Sound"]["stop"](ID), self["freshline"](); // 如果加载好了并且有文件正在播放，暂停
...
PlayMusic = function(b) { // 播放音乐
	let a = oAudio[b];
	if (a) a.stop(1), a.MaxLine = 1, oAudio[b].MinPlay = -Infinity, a.play(); // 先强制卸载
	else NewMusic(b), oAudio[b].MaxLine = 1, oAudio[b].MinPlay = -Infinity, oAudio[b].play();
},
PlayAudio = function(c, a) { // 播放音效
	let b = oAudio[c];
	b ? (a != null && b.loop(!!a), b.play()) : (NewAudio({ source: c, loop: !!a })).play()
},
